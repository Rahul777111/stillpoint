...
import {
  X, Play, Pause, SpeakerHigh, SpeakerSlash, LockSimple, ArrowsClockwise,
...
export function FocusOverlay({
  ...
  const begin = useCallback(async () => {
    ...
    setPhase('running');
    runTimer();
  }, [minutes, volume, world.layers, playVoice, runTimer]);

  const pause = useCallback(() => {
    clearTick();
    engine.setVolume(0.0001);
    setPhase('paused');
  }, []);
  ...
  const resume = useCallback(() => {
    ...
    setPhase('running');
    runTimer();
  }, [remaining, volume, runTimer]);

  // Switch scene mid-session without breaking the timer.
  ...
  // keep engine volume synced
  useEffect(() => {
    if (phase === 'running') engine.setVolume(volume);
  }, [volume, phase]);
    ...
    >
      ...
      <AnimatePresence mode="wait">
        <motion.img
          ...
          animate={{ opacity: 1, scale: reduce ? 1.05 : 1.12 }}
          exit={{ opacity: 0 }}
      ...
      <div className="absolute inset-x-0 top-0 z-10 flex items-center justify-between p-5 sm:p-7">
        ...
        <button
          ...
          className="glass flex h-11 w-11 items-center justify-center rounded-full text-mist transition-transform hover:scale-105"
        >
          <X size={20} />
        </button>
      </div>
          ...
          {phase === 'done' ? (
            ...
            >
              ...
              <h2 className="mt-3 font-display text-3xl font-semibold text-mist">
                {finishedMinutes} minutes of deep work.
              </h2>
              ...
              <div className="mt-6">
                <ShareCard minutes={finishedMinutes} world={world} stats={stats} />
              </div>
              <div className="mt-6 flex justify-center gap-3">
                <button onClick={() => { setPhase('setup'); setRemaining(minutes * 60); }} className="btn-ghost">
            ...
            <motion.div key="timer" className="flex flex-col items-center">
              ...
              <div className="relative">
                {!reduce && running && (
                  <span className="absolute inset-0 -z-10 animate-breathe rounded-full bg-ember/20 blur-2xl" />
                )}
                <svg width="300" height="300" viewBox="0 0 300 300" className="-rotate-90">
                  ...
                  <circle
                    cx="150" cy="150" r={R} fill="none" stroke="#E7B563" strokeWidth="4"
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="font-display text-6xl font-semibold tabular-nums text-mist">
                    {fmt(remaining)}
                  </span>
                  <span className="mt-1 text-sm text-mist-muted">
                    {phase === 'setup' ? 'ready when you are' : running ? 'in flow' : 'paused'}
                  </span>
                </div>
              </div>

              {/* Preset selector (setup only) */}
              {phase === 'setup' && (
                <div className="mt-9 flex flex-wrap justify-center gap-2">
                  {TIMER_PRESETS.map((p) => (
                    <button
                      key={p.minutes}
                      onClick={() => { setMinutes(p.minutes); setRemaining(p.minutes * 60); }}
                      className={`rounded-full border px-5 py-2.5 text-sm transition-all ${
                        minutes === p.minutes
                          ? 'border-ember bg-ember/15 text-ember'
                          : 'border-white/15 text-mist-muted hover:border-white/30 hover:text-mist'
                      }`}
                    >
              ...
              <div className="mt-8 flex items-center gap-4">
                {phase === 'setup' && (
                  <button onClick={begin} className="btn-primary px-9 py-4 text-base">
                    <Play size={20} weight="fill" /> Begin
                  </button>
                )}
                {running && (
                  <button onClick={pause} className="glass flex h-14 w-14 items-center justify-center rounded-full text-mist transition-transform hover:scale-105">
                    <Pause size={24} weight="fill" />
                  </button>
                )}
                {phase === 'paused' && (
                  <>
                    <button onClick={resume} className="btn-primary px-9 py-4 text-base">
                      <Play size={20} weight="fill" /> Resume
                    </button>
                    <button onClick={finish} className="btn-ghost">End early</button>
                  </>
                )}
              </div>
              ...
              <div className="mt-8 flex items-center gap-5">
                <button
                  ...
                  className="flex items-center gap-2 text-sm text-mist-muted transition-colors hover:text-mist"
                  aria-label="Toggle voice"
                >
                  {voiceOn ? <SpeakerHigh size={18} /> : <SpeakerSlash size={18} />}
                  voice
                </button>
                <div className="flex items-center gap-3">
                  ...
                  <input
                    ...
                    className="h-1 w-32 cursor-pointer appearance-none rounded-full bg-white/15 accent-ember"
                    aria-label="Volume"
                  />
                </div>
              </div>
            ...
            {worlds.map((w) => {
              ...
              return (
                <button
                  ...
                  className={`group relative h-16 w-28 shrink-0 overflow-hidden rounded-xl border transition-all ${
                    activeW ? 'border-ember' : 'border-white/10 hover:border-white/30'
                  }`}
                >
                  ...
                  {locked && (
                    <span className="absolute right-1.5 top-1.5 text-ember"><LockSimple size={14} weight="fill" /></span>
                  )}
                </button>
              );
            })}
